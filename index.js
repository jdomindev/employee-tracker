...
const updateEmployeeRole = () => {
  ...
  function (err, results) {
    ...
    function (err, results) {
      // console.log(results);
      rolesArray = results.map(result => result.title);
        const updateEmpRoleQuestions = [{ type: 'list', message: "Which employee's role would you like to update?", name: 'first_name', choices: employeeArray}, { type: 'list', message: 'Which role do you want to assign the selected employee?', name: 'role_title', choices: rolesArray}];
      inquirer
      ...
      .then((answers) =>
      {
        db.query('SELECT * FROM department WHERE department_name = ?', answers.department_name, function (err, results) {
          ...
          db.query('INSERT INTO role (title, salary, department_id) VALUES (?, ?, ?)', [answers.role, answers.salary, results[0].id], function (err, results) {
            // console.log(err);
            console.log(`Added ${answers.role} as a new role`)
            askQuestions();
          });
        });
      });
